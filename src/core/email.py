...
def send_welcome_with_verification_email(
    to: str,
    user_name: str | None,
    verification_url: str,
) -> str | None:
    """Send welcome + email verification in one message.

    Called after registration. Combines the friendly welcome tone with the
    required verification step — most products (GitHub, Notion, Figma) send
    a single combined email instead of two, which reduces the chance the
    verification link gets lost in inbox clutter.

    Args:
        to: recipient email
        user_name: user's full name (used in greeting; falls back to generic)
        verification_url: full URL to the verification page including the token
    """
    greeting = f"Привіт, {to_vocative(user_name)}!" if user_name else "Привіт!"
    html = f"""
    <!DOCTYPE html>
    ...
    <body style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; color: #1f2937;">
      ...
      <p>
        Щоб почати писати відгуки та пропонувати заклади, підтвердьте свою
        електронну пошту:
      </p>
      <p style="margin: 32px 0;">
      ...
      <p style="color: #6b7280; font-size: 14px;">
        Посилання діє 24 години. Якщо ви не реєструвалися на Pawly — просто
...
Щоб почати писати відгуки та пропонувати заклади, підтвердьте свою
електронну пошту, перейшовши за посиланням:

{verification_url}

Посилання діє 24 години. Якщо ви не реєструвалися на Pawly —
...
— Команда Pawly
    ...
    return send_email(
        to=to,
        subject="Ласкаво просимо до Pawly! Підтвердьте email 🐾",
        html=html,
        text=text,
    )
...
def send_verification_email(
    ...
    """Send a fresh verification email (no welcome wording)."""
    greeting = f"Привіт, {to_vocative(user_name)}!" if user_name else "Привіт!"
    html = f"""
    <!DOCTYPE html>
    <html>
    <body style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; color: #1f2937;">
      <h1 style="color: #111827;">Підтвердження email для Pawly</h1>
      <p>{greeting}</p>
      <p>
        Ось нове посилання для підтвердження вашої електронної пошти:
      </p>
      <p style="margin: 32px 0;">
        <a href="{verification_url}"
           style="background-color: #2563eb; color: white; padding: 12px 24px;
      ...
      <p style="color: #6b7280; font-size: 14px;">
        Посилання діє 24 години. Якщо ви не запитували це підтвердження —
        просто проігноруйте цей лист.
      </p>
      <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 32px 0;">
      <p style="color: #9ca3af; font-size: 12px;">— Команда Pawly</p>
    </body>
    </html>
    """
    text = f"""
...
— Команда Pawly
    ...
    return send_email(
        to=to,
        subject="Підтвердження email для Pawly",
        html=html,
        text=text,
    )
